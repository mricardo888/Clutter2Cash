...
export default function ActionPlacesScreen({ navigation, route }: Props) {
    ...
    const loadPlacesData = async () => {
        try {
            ...
            console.log('Item details received:', JSON.stringify(itemDetails, null, 2));

            // Check if actionPlaces exists in fullAnalysis
            if (itemDetails?.fullAnalysis?.actionPlaces) {
                const { selling, donation, recycling } = itemDetails.fullAnalysis.actionPlaces;

                console.log('Action places found:', { selling: selling?.length, donation: donation?.length, recycling: recycling?.length });

                // Add unique IDs to places if they don't have them
                if (selling && Array.isArray(selling)) {
                    setSellingPlaces(selling.map((place: Place, index: number) => ({
                        ...place,
                ...
                if (donation && Array.isArray(donation)) {
                    setDonationPlaces(donation.map((place: Place, index: number) => ({
                        ...place,
                ...
                if (recycling && Array.isArray(recycling)) {
                    setRecyclingPlaces(recycling.map((place: Place, index: number) => ({
                        ...place,
                ...
                console.log('Full analysis structure:', itemDetails?.fullAnalysis ? Object.keys(itemDetails.fullAnalysis) : 'No fullAnalysis');
            }
        } catch (error) {
            console.error("Error loading places data:", error);
            Alert.alert(
                "Error",
                "Failed to load places data. Please try again.",
                [{ text: "OK" }]
            );
        } finally {
                        ...
                        <Card key={place.id} style={styles.placeCard}>
                            <Card.Content>
                                ...
                                <View style={styles.actionButtons}>
                                    ...
                                    {place.type !== "selling" && place.address !== "Online Marketplace" && place.address !== "Mobile App" && (
                                        <Button
                                            mode="outlined"
