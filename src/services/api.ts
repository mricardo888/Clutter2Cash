...
const mockAnalysisData: AnalysisResponse[] = [
  ...
  },
  {
    item: "iPad Air",
    value: 300,
    ecoImpact: "35kg CO₂ saved",
    confidence: 0.87,
  },
  {
    item: "Dyson Vacuum",
    value: 150,
    ecoImpact: "20kg CO₂ saved",
    confidence: 0.85,
  },
  {
    item: "Instant Pot",
    value: 80,
    ecoImpact: "12kg CO₂ saved",
    confidence: 0.83,
  },
  {
    item: "AirPods Pro",
    value: 120,
    ecoImpact: "8kg CO₂ saved",
    confidence: 0.91,
  },
];
...
export class ApiService {
  ...
  static async analyzeItem(
    ...
    const randomItem =
      mockAnalysisData[Math.floor(Math.random() * mockAnalysisData.length)];

    // If text input provided, try to match it
    if (textInput) {
      const matchedItem = mockAnalysisData.find(
        (item) =>
